...
angular.module('angularLazyImg').factory('LazyImgMagic', [
  ...
  function($window, $rootScope, lazyImgConfig, lazyImgHelpers){
    ...
    function loadImage(photo){
      ...
      img.onload = function(){
        ...
        $rootScope.$apply(function () {
          ...
          if (typeof (photo._successCallback) === 'function') {
              photo._successCallback.call();
          }
        });
      };
      img.src = photo.src;
    ...
    Photo.prototype.setOnSuccess = function(cb){
  ...
  .directive('lazyImg', [
    '$rootScope', '$log', 'LazyImgMagic', function ($rootScope, $log, LazyImgMagic) {
      ...
      function link(scope, element, attributes) {
        ...
        scope.lazyImage.setOnSuccess(function () {
            scope.onLazySuccess();
        });
        scope.lazyImage.setErrorSource(attributes.lazyImgError);
        if(attributes.static){
            scope.lazyImage.setSource(attributes.lazyImg);
        }else {
